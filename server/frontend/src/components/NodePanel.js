...
function IfaceStatusDot({ online }) {
    if (online == null)
        return null;
    return (_jsxs("span", { className: `text-xs font-normal ${online ? "text-green-600" : "text-red-500"}`, children: ["\u25CF ", online ? "online" : "offline"] }));
}
// ---------------------------------------------------------------------------
...
function RnsConfigTab({ destHash, onLatLon }) {
    ...
    return (_jsxs("div", { className: "space-y-4", children: [rnsConfig && (_jsxs(_Fragment, { children: [_jsxs("div", { className: "rounded-lg border border-gray-200 p-4 space-y-2", children: [_jsx("h4", { className: "font-medium text-gray-700 text-sm", children: "[reticulum]" }), [
                        ...
                        if (type === "RNodeInterface")
                            return _jsx(RNodeCard, { iface: iface, onChange: (f) => updateIface(idx, f), online: online }, iface.name);
                        if (type === "TCPClientInterface")
                            return _jsx(TcpClientCard, { iface: iface, onChange: (f) => updateIface(idx, f), online: online }, iface.name);
                        if (type === "UDPInterface")
                            return _jsx(UdpCard, { iface: iface, onChange: (f) => updateIface(idx, f), online: online }, iface.name);
                        return _jsx(UnknownCard, { iface: iface, online: online }, iface.name);
...
export default function NodePanel({ destHash, node, onDelete, liveTelemetry }) {
    ...
    const [windowSec, setWindowSec] = useState(TIME_WINDOWS[1].sec); // 30m default
    ...
    useEffect(() => {
        ...
        const ts = liveTelemetry.timestamp ?? liveTelemetry.ts ?? Date.now() / 1000;
        const row = { ...liveTelemetry, ts };
    ...
    const spark = (field, label, unit, color, min, max) => {
        ...
        const latest = telemetry.find((r) => r[field] != null)?.[field] ?? null;
        const points = telemetry
            .filter((r) => r[field] != null)
            .map((r) => ({ ts: r.ts, value: r[field] }));
        return (_jsx(SparkLine, { points: points, domainStart: domainStart, domainEnd: domainEnd, label: label, unit: unit, latest: latest, color: color, min: min, max: max }, field));
    };
